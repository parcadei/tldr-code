...
//! - Tests stripped
//! - Visibility changed to pub for cross-module use
    ...
    for (e, v) in route {
        match e {
            ...
            ReplacedComment { levenshtein_pct } | ReplacedString { levenshtein_pct } => {
                ...
                // NOTE: 20% Levenshtein threshold. If levenshtein_pct <= 20,
                // replaced comments/strings are treated as Novel (Delete+Insert)
                // instead of ReplacedComment/ReplacedString (Update).
                // This is difftastic's hard-coded behavior.
